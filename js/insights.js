...
function get_metrics(tweets) {
  var all_metrics = tweets
    .map(function (tweet) {
      return {
        id: tweet.tweet.id_str,
        retweet_count: parseInt(tweet.tweet.retweet_count),
        favorite_count: parseInt(tweet.tweet.favorite_count),
      };
    })
    .sort(function (a, b) {
      return (
  ...
  let top_fav = all_metrics
    .sort((a, b) => {
      return b.favorite_count - a.favorite_count;
    })
    .map((t) => t.id)
    .slice(0, 10);
  let top_rt = all_metrics
    .sort((a, b) => {
  ...
  var total_favs = all_metrics
    .map(function (tweet) {
      return parseInt(tweet.favorite_count);
    })
    .reduce((partial_sum, a) => partial_sum + a, 0);
  var total_retweets = all_metrics
    .map(function (tweet) {
      return parseInt(tweet.retweet_count);
    })
    .reduce((partial_sum, a) => partial_sum + a, 0);

  top_fav = tweets.filter((tweet) => {
    return top_fav.indexOf(tweet.tweet.id_str) > -1;
  });
  top_rt = tweets.filter((tweet) => {
    return top_rt.indexOf(tweet.tweet.id_str) > -1;
  });
...
function get_tod(tweets) {
  ...
  var minutes = Object.keys(counts)
    .map(function (key) {
      return [key, counts[key]];
    })
    .sort();
...
document_ready(function () {
  ...
  let info_text = document.getElementById("insight_info");
  let p = document.createElement("p");
  p.innerHTML =
    "Your tweets received a total of <b>" +
    metrics.total_favs +
    " favourites</b> and <b>" +
